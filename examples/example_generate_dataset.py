...
# import helper_funcs_example_gen_dataset as hfunc
...
for p in (EXAMPLES_DIR, SCRIPTS_BLENDER):
...
FOCAL_LENGTHS = [1.5, 2.5] # [1.5, 2.5]
INTEROCULAR_DIST = [0.04] # , 0.08] # [0.04, 0.08]
...
hfunc.print_example_dataset_msg(CHOSEN_CAMERA, FOCAL_LENGTHS, INTEROCULAR_DIST, WATER_CONDITIONS, CLEAR_Z_OFFSETS, MURKY_Z_OFFSETS, MIN_OBJECTS, MAX_OBJECTS, NUM_RANDOM_ARRANGEMENTS, GRID_MIN, GRID_MAX)
# hfunc.get_example_confirmation()
...
for cam_obj in camera_collection.objects:
    ...
    # Only render for the chosen camera 
    if not hfunc.compare_camera(cam_obj, CHOSEN_CAMERA):
        continue
...
CHOSEN_CAMERA = "Approach_Retreat"

# Define depths for Ocean Volume
# Note: Offset of -25m in Blender = 0m depth e.g. -20m = 5m deep, -5m = 20m deep
CLEAR_Z_OFFSETS = [-20] # [-20, -5]
MURKY_Z_OFFSETS = [-23]
